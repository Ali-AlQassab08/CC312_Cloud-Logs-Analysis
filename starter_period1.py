...
def parse_line(line: str):
    ...
    """
    #1: strip whitespace and ignore empty lines (treat empty as invalid)
    line = line.strip()
    if not line:
        return None
    
    #2: split by '|' and trim whitespace around each part
    parts = [part.strip() for part in line.split('|')]
    
    # 3: if you do NOT have exactly 4 parts, return None
    if len(parts) != 4:
        return None
    
    # 4: return the 4 parts (timestamp, level, service, message)
    return tuple(parts)
